...
export class RoverInterpreter {
  ...
  private execute(command: RoverInterpreterCommands) {
    if(command === roverInterpreterCommands.F) return this.rover.turnRight();
    if (command === roverInterpreterCommands.B) return this.rover.turnRight();
    if (command === roverInterpreterCommands.L) return this.rover.turnRight();
